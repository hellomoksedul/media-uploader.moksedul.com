...
const MediaUploader: React.FC<MediaUploaderProps> = (props) => {
  ...
  return (
    <AlertDialog open={true} onOpenChange={onClose}>
      <AlertDialogContent className="max-w-[95vw]! h-[95vh]! gap-0 flex flex-col overflow-hidden p-4 lg:p-6">
        ...
        <>
          <div className="flex items-center justify-between shrink-0 pb-2">
            <AlertDialogTitle className="text-lg font-semibold">
              {isVideoUpload ? "Upload Video" : title}
            </AlertDialogTitle>
            <button
              onClick={onClose}
                ...
                {activeTab !== 5 && (
                  <div className="w-full flex justify-end pt-5 border-t border-border items-center gap-5 shrink-0">
                    <Button
                      ...
                      size="sm"
                      onClick={() =>
                        isCropping ? setIsCropping(false) : onClose()
                      }
                    >
                      {isCropping || isVideoUpload ? "Back" : "Cancel"}
                    </Button>
                    <Button
                      variant="default"
                      size="sm"
